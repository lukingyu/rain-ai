...
public class RuleBasedToolPlanner {
    ...
    public ToolPlan plan(AgentChatRequest request) {
        String message = request.message().toLowerCase(Locale.ROOT);
        if (containsAny(message, "知识库", "knowledge base", "kb") && containsAny(message, "列表", "有哪些", "列出", "查询")) {
            return new ToolPlan("knowledge_base.list", Map.of());
        }
        if (containsAny(message, "失败", "失败文档", "处理失败")) {
            UUID knowledgeBaseId = resolveKnowledgeBaseId(request.knowledgeBaseId());
            return new ToolPlan("document.failed.list", Map.of("knowledgeBaseId", knowledgeBaseId.toString()));
        }
        UUID knowledgeBaseId = resolveKnowledgeBaseId(request.knowledgeBaseId());
        return new ToolPlan("rag.ask", Map.of(
